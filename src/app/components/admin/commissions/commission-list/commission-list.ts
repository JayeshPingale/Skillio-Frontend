...
export class CommissionListComponent implements OnInit {
  ...
  totalItems = computed(() => this.filteredCommissions().length);
  totalPages = computed(() => Math.max(1, Math.ceil(this.totalItems() / this.itemsPerPage())));
  paginatedCommissions = computed(() => {
    const start = (this.currentPage() - 1) * this.itemsPerPage();
    return this.filteredCommissions().slice(start, start + this.itemsPerPage());
  });
  ...
  loadCommissions(): void {
    ...
    obs.subscribe({
      next: (data) => { this.commissions.set(data); this.currentPage.set(1); this.isLoading.set(false); },
  ...
  updateSearchTerm(value: string): void { this.searchTerm.set(value); this.currentPage.set(1); }
  updateStatus(value: string): void { this.selectedStatus.set(value); this.currentPage.set(1); }
